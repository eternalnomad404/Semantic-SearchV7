...
def render_query_view_screen(data: Dict):
    ...
    for result in query_results["expected_results"]:
        ...
        # Category emoji
        category_emoji = "🔧" if result["category"] == "tool" else (
...
def render_filters_analysis_screen(data: Dict):
    ...
    if filtered_results:
        ...
        styled_df = df.style.map(color_relevance, subset=['Relevance'])
        ...
        with col1:
            ...
            if high_percentage < 30:
                ...
                st.info("✅ High proportion of highly relevant items. Ensure quality standards are maintained.")
        ...
        with col2:
            ...
            if low_percentage > 50:
                ...
                st.info("✅ Good precision. Most results are relevant.")
